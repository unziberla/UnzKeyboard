...
public class PhraseGeneratorVer extends PhraseGenerator {
	private Vector<Vector<String> > phrases;
	private int index, set;
	
	public PhraseGeneratorVer() {
		...
		phrases.get(1).add("I like baroque and classical music");
		...
		phrases.get(2).add("the accident scene is a shrine for fans");
		...
		phrases.get(3).add("a coupon for a free sample");
		...
		phrases.get(3).add("the plug does not fit the socket");											
		
	}			
				
	public String getPhrase() {
		String s;
		if (index == 10 || (set == 3 && index == 5))
		{
			index = 0;
			set++;
		}
